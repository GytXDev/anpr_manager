...
class AnprPeageController(http.Controller):
    ...
    def _send_to_remote_odoo(self, ref, date, amount, plate, journal_code, debit_code, credit_code):
        ...
        remote_url = "https://devotech.archisec-it.com"
        db = "bdproduction"
        login = "n.leyalangoye@ogoouetech.com"
        password = "Ogooue1234"
        ...
        journal_id = models.execute_kw(db, uid, password, 'account.journal', 'search', [[('code', '=', journal_code)]])[0]
        debit_id = models.execute_kw(db, uid, password, 'account.account', 'search', [[('code', '=', debit_code)]])[0]
        credit_id = models.execute_kw(db, uid, password, 'account.account', 'search', [[('code', '=', credit_code)]])[0]
        ...
        move_id = models.execute_kw(db, uid, password, 'account.move', 'create', [{
            ...
            'line_ids': [
                (0, 0, {
                    'name': f"[DISTANT] Paiement péage {plate}",
                    'account_id': debit_id,
                    'debit': amount,
                    'credit': 0.0,
                }),
                (0, 0, {
                    'name': f"[DISTANT] Recette péage {plate}",
    ...
    def process_manual_payment(self, plate, vehicle_type, amount):
        ...
        try:
            ...
            # ➜ Création comptable locale + backup distant
            self._create_account_move(amount, payment_method, plate, user.name)

            # ➜ Impression (une seule fois ici)
            try:
                print_data = {
                ...
                _logger.warning(f"[IMPRESSION] Erreur d’impression : {e}")
        ...
        try:
            ...
            if payment_status == 'success':
                ...
                try:
                    ...
                    _logger.warning(f"[IMPRESSION] Erreur d’impression mobile : {e}")
